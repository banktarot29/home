...
- observationsText ใส่ 4 ข้อ คั่นด้วย | ทุกข้อต้องขึ้นต้นด้วยหัวข้อสั้น 2-6 คำ ตามด้วยเครื่องหมาย : เสมอ ห้ามละเว้นแม้แต่ข้อเดียว แล้วตามด้วยรายละเอียด แต่ไม่ใช่เช็กลิสต์ลอยๆ ให้ตอบว่าบ้านกำลังบอกอะไรกับเจ้าของ และอาการที่มักพบร่วมกับบ้านลักษณะนี้ ใช้คำว่า มักพบร่วมกับ ห้ามสรุปว่าเป็นแน่นอน
...
ถ้าภาพหน้าบ้านเห็นรายละเอียด เช่น ประตู ทางเดิน รั้ว พื้น ชาน ทางลาด ของวาง ตู้ไฟ สายไฟ หลังคา ความทึบ ความโปร่ง ให้ดึงสิ่งนั้นมาเขียนโดยตรง ห้ามตอบแบบกว้างๆ`;
  ...
  // v15.1: จับเวลาจริงไว้ดูใน logs — เฝ้าระวังว่าเคส 2 ภาพเข้าใกล้เพดาน 57s แค่ไหน
  const _t0 = Date.now();
  ...
  try {
    ...
    console.log(`[analyze] ok in ${Date.now() - _t0}ms images=${interiorB64 ? 2 : 1} via=${via}`);

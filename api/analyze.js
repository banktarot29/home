...
ถ้าต้องแบ่งหลายข้อ ให้คั่นแต่ละข้อด้วยเครื่องหมาย | ใน string เดียว
...
วิธีเขียน:
- headline สรุปแบบมืออาชีพ 1 ประโยค ไม่เกิน 80 ตัวอักษร
...
- frontPart 2-3 ประโยค ต้องเจาะจากภาพจริง และบอกให้เห็นภาพว่าเจ้าของบ้านควรเริ่มมองจุดไหนก่อน
- interiorPart ถ้าไม่มีภาพภายใน ให้บอกตรงๆ ว่ายังไม่ได้ประเมินภายใน และไม่ควรสรุปแทนหน้างาน
- observationsText ใส่ 4-6 ข้อ คั่นด้วย | ทุกข้อต้องอ้างสิ่งที่เห็นจริง เช่น ชานยกระดับ ทางลาด ประตูบานเลื่อน ประตูม้วน หลังคาเหล็ก ของวางสองข้าง ตู้ไฟ/สายไฟ เลขห้อง เสาอิฐ แสง ความทึบ ทางเดินเข้าบ้าน
- goodText ใส่ 2-3 ข้อ คั่นด้วย | ต้องเป็นจุดดีที่เห็นจริง ไม่ชมลอยๆ และใช้คำที่คนทั่วไปเข้าใจง่าย
- badText ใส่ 4-6 ข้อ คั่นด้วย | ต้องเป็นจุดเสี่ยงที่เห็นจริง พร้อมเหตุผลสั้นๆ ว่าทำไมควรปรับ
- fixText ใส่ 7-8 ข้อ คั่นด้วย | ข้อ 1-2 เป็นวิธีที่เจ้าของบ้านทำเองได้ทันที แบบไม่ทุบ ไม่รื้อ และเขียนให้เห็นภาพว่าต้องทำอย่างไร เช่น เก็บอะไร ย้ายอะไร เปิดทางเดินตรงไหน ข้อที่เหลือเป็นประเด็นที่ควรให้ซินแสดูต่อ
- omen เขียนภาพรวม 3-4 ประโยค อ่านลื่น ชี้นำให้เห็นว่าควรปรับอย่างเป็นระบบ แต่ไม่การันตีผลลัพธ์ ปิดท้ายว่าการวิเคราะห์อาจคลาดเคลื่อนได้เพราะไม่ได้ไปดูหน้างานจริง
...
function normalizeAiResult(result, hasInterior) {
  ...
  const splitItems = (value, fallbackTitle, max) => {
    ...
    return parts.map((desc, index) => ({ title: fallbackTitle + ' ' + (index + 1), desc }));
  };
  ...
  return {
    score,
    headline: cleanText(result.headline) || 'บ้านหลังนี้ควรปรับหน้าบ้านให้โปร่งและเป็นระบบขึ้น',
    ...
    observations: observations.length ? observations : [{ title: 'สิ่งที่เห็นจากภาพ', desc: 'เห็นหน้าบ้านและทางเข้าหลัก แต่รายละเอียดบางส่วนอาจต้องตรวจหน้างานเพิ่มเติม' }],

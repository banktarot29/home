...
- goodText ใส่ 3 ข้อ
- badText ใส่ 4 ข้อ
- fixText ใส่ 7-8 ข้อ ข้อ 1-2 เป็นสิ่งที่ทำเองได้ทันที ข้อถัดไปเป็นหัวข้อเฉพาะที่ควรส่งรูปเพิ่มหรือให้ซินแสดูต่อ เช่น “ตำแหน่งประตูหลัก”, “ระดับพื้นหน้าบ้าน”, “รั้วและแนวทางเข้า”, “ต้นไม้และของบังสายตา”, “สีและวัสดุหน้าบ้าน”, “ตู้ไฟ สายไฟ และของหนักด้านหน้า”, “มุมถ่ายหน้าบ้านเพิ่มเติม”
...
ห้ามพูดถึงภายในบ้านในผลหลัก หากต้องใส่ช่อง interiorPart ให้ระบุเพียงว่ารอบนี้โฟกัสหน้าบ้านเท่านั้น`;
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            { type: 'text', text: 'สรุปเป็นหลักฐานหน้าบ้านสั้น กระชับ ตรง ไม่แต่งเรื่อง ถ้าภาพไม่พอให้บอกว่าควรถ่ายมุมไหนเพิ่ม และระบุเหตุผลที่เหมาะกับการชวนส่งรูปเพิ่ม/คุยต่อโดยไม่ขายแข็ง' }
          ]
        }]
      })
    });
    ...
    const visualEvidence = evidenceToolUse?.input || {
      ...
      interiorSeen: 'รอบนี้ไม่ตรวจภาพภายในบ้าน',
      interiorRisks: 'ไม่ใช้ประเด็นภายในบ้านในการให้คะแนนรอบนี้',
      linkBetweenPhotos: 'ใช้เฉพาะภาพหน้าบ้านเป็นฐานในการสรุป',
      missingAngles: 'ควรถ่ายหน้าบ้านตรง ๆ จากฝั่งตรงข้ามหรือหน้ารั้ว และถ่ายมุมเฉียงให้เห็นทางเดินกับประตูหลัก',
      confidence: 'ปานกลาง',
...
- ขอบเขตการวิเคราะห์: ${visualEvidence.linkBetweenPhotos}
...
- เหตุผลเชิงปฏิบัติที่ควรชวนส่งรูป/คุยต่อ: ${visualEvidence.conversionReason}`;
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: 'claude-sonnet-4-20250514',
        max_tokens: 3600,
        system: frontOnlySystemPrompt + '\n\nกติกาเพิ่มเติมสำหรับรอบเขียนผล: เขียนแบบมือโปร ตรง ใช้งานได้จริง ไม่อวยเกินจริง ไม่ขายแข็ง ทุกข้อสำคัญต้องอ้างหลักฐานหน้าบ้านหรือข้อจำกัดจากหลักฐานด้านล่างก่อนเสมอ ถ้าภาพยังไม่พอให้ชวนส่งรูปเพิ่มด้วยเหตุผลเฉพาะจุด ไม่ใช้ประโยคขายทั่วไป',
        tools: [analysisTool],
...
ให้เขียนผลวิเคราะห์จากหลักฐานหน้าบ้านนี้เป็นหลัก ห้ามวิเคราะห์ภายในบ้าน ห้ามพูดเหมือนเห็นพื้นที่ที่ภาพไม่ได้แสดง และถ้าภาพหน้าบ้านยังไม่ครบให้ระบุมุมที่ต้องถ่ายเพิ่มแบบสั้นชัด
